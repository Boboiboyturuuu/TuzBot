...
var authFolder = storeSystem.fixFileName(`${Helper.opts._[0] || ''}Turuuboy`)
...
function clearSessions(folder = "Turuuboy") {
    let filename = [];
...
function purgeOldFiles() {
    ...
    const directories = ['./Turuuboy/', './jadibot/'];
...
async function connectionUpdate(update) {
   ...
   📝 Deskripsi: *Bot ${name || 'TuruBoy'} sudah aktif*.
        `;
...
global.reloadHandler = async function(restatConn) {
    ...
    conn.welcome = `${emoji.welcome} Hallo dek/kk/tante/om@user\n\n   *W E L C O M E*\n⫹⫺ Di grup @subject\n\n⫹⫺ Baca *DESKRIPSI*\n@desc`;
    conn.bye = `   *G O O D B Y E*\n${emoji.bye} Anak h4r4m @user`;
    ...
    conn.sAnnounceOn = `${emoji.announceOn} Grup telah ditutup admin mau colay dulu!\nSekarang hanya admin yang dapat mengirim pesan.`;
    conn.sAnnounceOff = `${emoji.announceOff} Grup telah dibuka admin sudah selesai colay!\nSekarang semua peserta dapat mengirim pesan.`;
